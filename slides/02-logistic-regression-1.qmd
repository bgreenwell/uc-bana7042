...
* For a fair coin :coin:, the probability of getting tails is $p = 0.5$. Therefore, the odds of getting tails vs. heads is $p / (1 - p) = 0.5 / 0.5 = 1$. (We might also say the odds of getting tails is "1 to 1" or "1:1").
...
* Recall that if $Y_i \stackrel{iid}{\sim} Bernoulli\left(p\right)$, then the [likelihood function](https://en.wikipedia.org/wiki/Likelihood_function) is defined as
...
Lots of ways to do this (some bad, some better). R defaults to using a [profile likelihood](https://en.wikipedia.org/wiki/Likelihood_function) method:
